...
public class ClockGUI extends Applet implements KeyListener, ClockTimeDisplay, ItemListener {
	...
	private int IncTime(int time, int pos) {
		int newtime = time;
		if ((pos == 5 && GetDigit(time, 5) < 2 && GetDigit(time, 4) < 4) || (pos == 5 && GetDigit(time, 5) < 1)
				|| (pos == 4 && GetDigit(time, 4) < 3 && GetDigit(time, 5) <= 2)
				|| (pos == 4 && GetDigit(time, 4) < 9 && GetDigit(time, 5) <= 1) || (pos == 3 && GetDigit(time, 3) < 5)
				|| (pos == 1 && GetDigit(time, 1) < 5) || ((pos == 0 || pos == 2) && GetDigit(time, pos) < 9)) {

...
public class ClockInput {
	...
	/**
	 * Semaphore signaling when the user have changed any setting. Actually,
	 * according to normal rules for data abstraction, data (such as the flags
	 * and the semaphore in this class) should be accessed via methods, say
	 * awaitAnyButton() to take the semaphore. However, here we expose variables
	...
	/**
	 * When getChoice returns a new choice, and the previous choice was either
	 * SET_ALARM or SET_TIME, the set-value of the display is returned in the
	 * format hhmmss where h, m, and s denotes hours, minutes, and seconds
	 * digits respectively. This means, for example, that the hour value is
	 * obtained by dividing the return value by 10000.
	 */
	public int getValue() {
		return lastValueSet;
	}
